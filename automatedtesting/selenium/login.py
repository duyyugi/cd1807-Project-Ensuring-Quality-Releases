...
def login (user, password):
    ...
    print (timestamp() + 'user ' + user + ' login test successfully')
...
def add_and_remove_items_to_cart(driver):
    ...
    for item in items:
        print (timestamp() + item.text + "is added to cart")
        syslog.syslog(item.text + ' login test successfully')
    for button in add_button_lists:
        button.click()
    for item in items:
        print (timestamp() + item.text + "is removed from cart")
        syslog.syslog(item.text + "is removed from cart")
    for button in add_button_lists:
        button.click()
    
    print (timestamp() + 'add and remove test successfully')
